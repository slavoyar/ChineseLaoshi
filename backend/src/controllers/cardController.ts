...
export const createCard = async (req: CustomRequest<Card, CreateCardRequest>, res: Response<Card>) => {
...
export const updateCard = async (req: CustomRequest<Card, UpdateCardRequest>, res: Response) => {
  const card = await prisma.card.findFirst({ where: { id: req.params.cardId }, include: { word: true } })
  if (!card) {
