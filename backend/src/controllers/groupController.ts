...
export const getAllGroups = async (req: Request, res: Response) => {
  ...
  const groups = await prisma.group.findMany({ where: { userId: req.user.id } });
  res.status(200).json(groups);
}

export const createGroup = async (req: CustomRequest<Group, { name: string }>, res: Response) => {
  // @ts-ignore
  const group = await prisma.group.create({ data: { name: req.body.name, userId: req.user?.id } })
  res.status(200).json(group);
}
export const updateGroup = async (req: CustomRequest<Group, Group>, res: Response) => {
  const group = await prisma.group.update({ where: { id: req.params.groupId }, data: { name: req.body.name } })
  res.status(200).json(group);
}
