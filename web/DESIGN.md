...
Chinese Laoshi is a study tool, not a dashboard. The interface is a dark, quiet stage where Mandarin characters take center position and everything else тАФ navigation, metadata, actions тАФ stays in supporting roles. Built on shadcn/ui (slate base) with Tailwind CSS variables, the system favors familiar product patterns over decorative novelty: square tiles for vocabulary, bordered content panels, Lucide icons, and state-rich controls that feel trustworthy on phone, tablet, and desktop.

Dark mode is the default experience (`class="dark"` on `<html>`). Surfaces are flat and tonal; depth comes from border contrast and layered neutrals, not heavy shadows. Motion is brief (150тАУ200 ms) and functional тАФ hover reveals, dialog enter/exit, skeleton pulse тАФ always gated behind `prefers-reduced-motion`.
...
- **Characters first** тАФ Chinese glyphs render at `text-2xl` on word tiles; English metadata stays micro-sized beneath.
- **Tile-native layout** тАФ Groups and words share a 96тАУ104 px square tile grid with consistent internal structure.
- **Restrained accent** тАФ Primary (inverted light-on-dark) drives actions; green signals mastery progress only.
- **Task density without noise** тАФ Content width capped (`md:w-9/12`, `xl:w-7/12`); panels use generous padding without nested cards.
- **Touch-ready targets** тАФ Tiles, icon buttons (40 px), and header controls sized for finger input.
...
- **Stage Light** (#f8fafc / hsl(210 40% 98%)): Primary buttons, focus-adjacent highlights, and the inverted accent in dark mode. Used for constructive committed actions (Create, Save) тАФ not decoration, and not irreversible delete.
...
- **Supporting Gray** (#94a3b8 / hsl(215 20.2% 65.1%)): Secondary copy тАФ transcriptions, word counts, placeholders, back links. Must remain readable on secondary surfaces (тЙе4.5:1 against #1e293b).
- **Focus Ring** (#cbd5e1 / hsl(212.7 26.8% 83.9%)): 2 px focus-visible ring on interactive elements.
- **Danger Deep** (#7f1d1d / hsl(0 62.8% 30.6%)): Destructive button fill. Tile delete rests at Supporting Gray and only brightens to red (#f87171 / red-400) on hover, focus, or press тАФ so always-visible touch chrome does not outrank characters.

### Named Rules (optional, powerful)

**The Character Spotlight Rule.** Primary and green accents combined occupy тЙд15% of any study screen. If color is everywhere, characters stop feeling like the hero.
...
**Label/Mono Font:** None тАФ single-family product UI
...
- **Title** (regularтАУbold, 1.5rem / 24px, line-height 1.25): Page and section headings тАФ app name in header (`font-bold`), "Groups", editable group names.
...
**The Character Scale Rule.** Chinese glyphs on tiles must be at least 1.5rem. Supporting Latin/Cyrillic metadata stays at micro or label sizes beneath тАФ never the same size as the character.
...
**The Flat Stage Rule.** Surfaces at rest have no shadow. Elevation appears only as a modal response тАФ never on tiles, list rows, or content panels.

## 5. Components

Product UI built on shadcn/ui + Radix primitives. Lucide icons at 16 px (`h-4 w-4`) in controls, 24тАУ28 px in tiles.
...
- **Header:** Fixed 80px height, bottom border, `bg-card`. App name (ф╕нхЫ╜шАБх╕И) as bold 1.5rem button left; user chip with circular primary avatar (32px icon wrapper) right.
...
- **Close:** Absolute top-right X, opacity 70 тЖТ 100 hover.

### Skeleton / Loading

- **Tile skeleton:** Matches tile dimensions тАФ secondary bordered shell, pulse rectangles for icon and text lines.
...
- **Do** apply green only through `getProgressStyles()` semantics for word mastery тАФ border, line, and label together.
...
- **Don't** use childish gamification тАФ cartoon mascots, excessive badges, Duolingo-style streak guilt, or babyish color palettes.
- **Don't** apply generic SaaS dashboard aesthetics to study screens тАФ no hero metrics, no gradient accents, no decorative chart colors on non-analytics views.
- **Don't** clutter interfaces so chrome competes with characters тАФ avoid nested cards, redundant eyebrows, or side-stripe borders on tiles.
- **Don't** use gradient text, glassmorphism, or heavy shadows on tiles and panels.
- **Don't** gate delete or edit icons on hover alone тАФ touch and other no-hover devices must see them without a pointer hover.
...
- **Don't** gate content visibility on entrance animations тАФ default state must be fully readable without JS-driven reveals.
