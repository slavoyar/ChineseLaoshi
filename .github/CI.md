...
```text
PR → production
  CI — Frontend / check     (lint + Next web build)
...
- **`check`** — `npm run lint`, `npm run build` in `web/`
- **`e2e`** — Playwright tests (starts embedded backend + Next `:3001`)
...
CI=1 JWT_SECRET=e2e-ci-jwt-secret GOOGLE_CLIENT_ID=e2e-google-client-id COOKIE_SECURE=false npm run test:e2e --workspace=@chinese-laoshi/web
